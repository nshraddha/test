...
dust.helpers.strConcat = function(chunk, context, bodies, params) {
  var returnStr = "";
  for(var key in params){
    returnStr += params[key];
  }
  return chunk.write(returnStr);
} ; 
dust.helpers.jdata = function(chunk, context, bodies, params) {
  var returnStr = "";
  //log(dust.helpers.tap(params.id, chunk, context) , dust.helpers.tap(params.data, chunk, context));
  //$(document).data(dust.helpers.tap(params.id, chunk, context), dust.helpers.tap(params.data, chunk, context));
  localStorage[dust.helpers.tap(params.id, chunk, context)] = dust.helpers.tap(params.data, chunk, context) ;
  return chunk.write(returnStr);
};
dust.helpers.log = function(chunk, context, bodies, params) {
  ...
  //log(dust.helpers.tap(params.id, chunk, context) , dust.helpers.tap(params.data, chunk, context));
  var txt = params.txt || "";
  log("log from template "+txt , dust.helpers.tap(params.obj, chunk, context));

  return chunk.write(returnStr);
}  ; 
dust.helpers.len = function(chunk, context, bodies, params) {
  var returnStr = 0;
  var item = dust.helpers.tap(params.item, chunk, context);
  if( item instanceof Array) {
    returnStr = item.length;
  }
  return chunk.write(returnStr);
} ; 
dust.helpers.tostr = function(chunk, context, bodies, params) {
  var returnStr = 0;
  var item = dust.helpers.tap(params.item, chunk, context);
  if( item instanceof Array) {
    returnStr = item.join("&#10;");
...
dust.helpers.stringify = function(chunk, context, bodies, params) {
  var returnStr = "";
  returnStr = JSON.stringify(params['obj']);
  return chunk.write(returnStr);
}   ; 
dust.helpers.up = function(chunk, context, bodies, params) {
  var returnStr = "";
  var str = dust.helpers.tap(params.str, chunk, context);
  if(typeof str != "undefined") returnStr = (str+"").toUpperCase();
  return chunk.write(returnStr);
} ;
dust.helpers.upTrim = function(chunk, context, bodies, params) {
  var returnStr = "";
  var str = dust.helpers.tap(params.str, chunk, context);
  if(typeof str != "undefined") returnStr = (str+"").toUpperCase();
  if(returnStr && returnStr.length>35) returnStr = returnStr.substring(0,35).trim()+" ...";
  return chunk.write(returnStr);
} ; 
dust.helpers.replaceStr = function(chunk, context, bodies, params) {
  var returnStr = "";
  var str = dust.helpers.tap(params.str, chunk, context);
  var rep = dust.helpers.tap(params.replace, chunk, context);
  var wit = dust.helpers.tap(params.with, chunk, context);
  try {
    returnStr = str.replace(rep, wit);
  }catch(e){
    log("replace helper broke", e.toString());
  }
  return chunk.write(returnStr);
};
dust.helpers.removeSpaces = function(chunk, context, bodies, params) {
  var returnStr = "";
  var str = dust.helpers.tap(params.str, chunk, context);
  try {
    returnStr = str.replace(/\s/g, '');
  }catch(e){
    log("removeSpaces helper broke", e.toString());
  }
  return chunk.write(returnStr);
};

var removeObject = function(input){
  if(!input){
    return "";
  }
  var positionOfObject = input.indexOf("[object Object]");
  if(positionOfObject!=-1){
    input=input.substring(0,positionOfObject);
  }
  return input;
};
dust.helpers.isAuth = function(chunk, context, bodies, params) {
    var returnStr = "";
    var permission = dust.helpers.tap(params.permission, chunk, context);
    if(permission){
      permission = permission.toLowerCase();
      var mode = dust.helpers.tap(params.mode, chunk, context) || "W";
      var project = dust.helpers.tap(params.project, chunk, context);
      if(typeof isAuthorized =="function" && isAuthorized(permission,mode,project)){
        returnStr = bodies.block(chunk, context);
      }
    }else{
      returnStr = bodies.block(chunk, context);
    }
    var output = chunk.write(returnStr);
    output.data = removeObject(output.data);
    // log("output",output.data);
    return output;
};
dust.helpers.isNotAuth = function(chunk, context, bodies, params) {
    var returnStr = "";
    var permission = dust.helpers.tap(params.permission, chunk, context);
     if(permission){
      permission = permission.toLowerCase();
...
dust.helpers.getCustomLabel = function(chunk, context, bodies, params) {
    var returnStr = "";
    var label = dust.helpers.tap(params.label, chunk, context);
    var defaultLabel = dust.helpers.tap(params.default_label, chunk, context);
    returnStr = cloudmunch && cloudmunch.getCustomLabel && cloudmunch.getCustomLabel(label,defaultLabel);
    return chunk.write(returnStr);
};
// {@userID input="{?value.username}{value.username}{:else}System{/value.username}"}
var removeAfterAt = function(inputContent){
  var returnValue = inputContent;
  if(inputContent.indexOf("@")!=-1){
    returnValue= inputContent.substring(0,inputContent.indexOf("@"));
  }
  return returnValue;
};

dust.helpers.userID = function(chunk, context, bodies, params) {
    var returnStr = "";
    var originalContent = dust.helpers.tap(params.input, chunk, context);
    var newContent =  dust.helpers.tap(params.defaultValue, chunk, context);
    if(originalContent){
      newContent = (originalContent == cloudmunch.getData('username'))?'you':removeAfterAt(originalContent);
    }
    returnStr = newContent;
    return chunk.write(returnStr);
};
